...
import sys #utils error
sys.path.append('/home/amna.alhosani/DecompDiff/')
...
def eval_single_datapoint(index, id, args):
    ...
    for sample_idx, sample_dict in enumerate(tqdm(index[:num_samples], desc='Eval', total=num_samples)): # Loop iterates over the samples to evaluate 
        if sample_dict['mol'] is None: # Reconstruct the molecule from predicted atomic position, bond index, bond type. 
            try:
            ...
            except: #If it fails, molecule and smiles are set to none. 
                logger.warning('Reconstruct failed %s' % f'{sample_idx}')
                mol, smiles = None, None
        else: # If successful, molecule and smiles string are stored 
            mol = sample_dict['mol']
            smiles = sample_dict['smiles']

        if mol is None or '.' in smiles: # If mol: None / Smiles: '.', loop is skipped
            continue
...
if __name__ == '__main__':
    ...
    r, pd, bd = eval_single_datapoint(test_index[:args.eval_num_examples][0], list(range(args.eval_num_examples))[0], args)
