...
if __name__ == '__main__':
    ...
    if len(args.ckpt_path) > 0:
        ...
        scheduler_ckpt = ckpt["scheduler"]
        iteration_ckpt = ckpt["iteration"]
    ...
    def train(it):
        ...
        try:
            for _ in range(config.train.n_acc_batch):
                ...
                protein_noise = torch.randn_like(batch.protein_pos) * config.train.pos_noise_std # creating noise vector
                gt_protein_pos = batch.protein_pos + protein_noise # adding noise to input data
                prior_centers_noise = torch.randn_like(batch.ligand_decomp_centers) * config.train.prior_noise_std # creating noise vector
                prior_centers = batch.ligand_decomp_centers + prior_centers_noise # adding noise to prior center
    ...
    try:
        ...
        for it in range(iteration_ckpt, config.train.max_iters + 1):
