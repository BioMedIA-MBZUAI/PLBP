...
def center_pos(protein_pos, ligand_pos, batch_protein, batch_ligand, mode='protein'):
    if mode == 'none':
        ...
        offset = scatter_mean(protein_pos, batch_protein, dim=0) # calculates mean position of all protein positions
        protein_pos = protein_pos - offset[batch_protein] # each protein position is subtracted by calculated offset 
        if ligand_pos is not None:
            ligand_pos = ligand_pos - offset[batch_ligand] # shift ligand along with the protein (aligned)
        # print('offset: ', offset)
    else:
...
class DecompScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim, num_classes,
        ...
        # reading from config (training.yml)
    ...
    def forward(self, protein_pos, protein_v, batch_protein, protein_group_idx,
                ...
                ligand_fc_bond_index, init_ligand_fc_bond_type, # bond info
                ligand_atom_mask=None, time_step=None, return_all=False):

        init_ligand_v = F.one_hot(init_ligand_v, self.num_classes).float() # initial features of ligand atoms (one hot encoded)
        ...
        h_protein = self.protein_atom_emb(protein_v) # embedded protein features
        init_ligand_h = self.ligand_atom_emb(input_ligand_feat) # embedded ligand features

        if self.add_prior_node: # Its (False)
            prior_std_feat = self.prior_std_expansion(prior_stds) # skipped
            prior_h = self.prior_atom_emb(prior_std_feat) # skipped 
        ...
        if self.config.node_indicator:
            if self.add_prior_node:
                ...
                prior_ind = torch.tensor([0, 0, 1]).unsqueeze(0).repeat(len(prior_h), 1).to(h_protein) # skipped
                prior_h = torch.cat([prior_h, prior_ind], -1) # skipped
            else: # model only uses protein and ligand embedded features
                pro_ind = torch.tensor([0]).unsqueeze(0).repeat(len(h_protein), 1).to(h_protein) # protein node indicator
                lig_ind = torch.tensor([1]).unsqueeze(0).repeat(len(init_ligand_h), 1).to(h_protein) # ligand node indicator
        ...
        if self.add_prior_node:
            ...
            h_all, pos_all, batch_all, mask_ligand, mask_ligand_atom, p_index_in_ctx, l_index_in_ctx = compose_context(
                h_protein=h_protein, # protein embedded representations
                h_ligand=init_ligand_h, # ligand embedded representations
                pos_protein=protein_pos, # protein positional embeddings
                pos_ligand=init_ligand_pos, # ligand positional embeddings
                batch_protein=batch_protein,
                batch_ligand=batch_ligand,
                ligand_atom_mask=ligand_atom_mask # highlights valid ligand atoms 
            )
            group_idx_all = None

        if ligand_fc_bond_index is not None:
            bond_index_in_all = l_index_in_ctx[ligand_fc_bond_index] # maps indices within ligands in(ligand_fc_bond_index) to their corresponding positions within the combined context
        else:
            bond_index_in_all = None

        if self.refine_net_type == 'uni_o2_bond': # model type
            bond_type = F.one_hot(init_ligand_fc_bond_type, num_classes=self.num_bond_classes).float() # converts integer bond types it into one-hot encoded representations
            h_bond = self.ligand_bond_emb(bond_type) # embedded representation of bond type
        ...
        final_pos, final_h = outputs['x'], outputs['h'] # updated positional info , updated embedded representation
        ...
        if self.bond_diffusion: # True
            ...
            if self.bond_net_type == 'pre_att':
                ...
                if self.bond_net_type == 'pre_att':
                    ...
                    raise NotImplementedError
            elif self.bond_net_type == 'lin': # this will run
    ...
    def get_diffusion_loss(
        ...
        if time_step is None:
            ...
            pt = torch.ones_like(time_step).float() / self.num_timesteps
        a = self.alphas_cumprod.index_select(0, time_step)  # (num_graphs, ) dimensions=4

        # 2. perturb pos, v, (and bond)
        assert len(ligand_decomp_batch) == prior_num_atoms.sum().item() # if it matches tot num of atoms in the decomposed prior
        ...
        pos_noise = torch.zeros_like(ligand_pos) # noise tensor
        pos_noise.normal_() # normal noise distribution
        # Xt = a.sqrt() * X0 + (1-a).sqrt() * eps
        ligand_pos_perturbed = a_pos.sqrt() * (ligand_pos - batch_noise_centers) + \
                               (1.0 - a_pos).sqrt() * pos_noise * batch_noise_stds + batch_noise_centers # perturbed ligand positions
        # Vt = a * V0 + (1-a) / K
        ...
        if self.bond_diffusion:
            ...
            ligand_b_perturbed, log_ligand_bt = self.bond_type_trans.q_v_sample( # perturbed bond types
                log_ligand_b0, time_step, batch_ligand_bond)
        else:
            ligand_b_perturbed = None

        protein_pos, ligand_pos_perturbed, offset = center_pos(
            protein_pos, ligand_pos_perturbed, batch_protein, batch_ligand, mode=self.center_pos_mode) # mode is protein:
        ...
        pred_pos_noise = pred_ligand_pos - ligand_pos_perturbed # predicted position noise 
        # atom position
        if self.model_mean_type == 'noise':
            ...
            pos_model_mean = self.q_pos_posterior(
                x0=pred_ligand_pos, xt=ligand_pos_perturbed, t=time_step, batch=batch_ligand) # mean is predicted ligand positions
        else:
            raise ValueError
        # atom type
        log_ligand_v_recon = F.log_softmax(pred_ligand_v, dim=-1) # reconstructed logarithm ligand velocities
        log_v_model_prob = self.atom_type_trans.q_v_posterior(log_ligand_v_recon, log_ligand_vt, time_step, batch_ligand) # model posterior probabilities 
        log_v_true_prob = self.atom_type_trans.q_v_posterior(log_ligand_v0, log_ligand_vt, time_step, batch_ligand) # true posterior probabilities 
        ...
        kl_v = self.compute_v_Lt(log_v_model_prob=log_v_model_prob, log_v0=log_ligand_v0, # KL divergence 
                                 log_v_true_prob=log_v_true_prob, t=time_step, batch=batch_ligand)
        if self.bond_diffusion: #similar to the above --> predicting bond types and estimating KL divergence
        ...
        if self.bond_diffusion:
            results['losses']['bond'] = loss_bond # KL divergence 
