...
class NodeUpdateLayer(nn.Module):
    ...
    def forward(self, h, edge_feat, edge_index, e_w=None):
        ...
        # existing_alpha = np.load("attention_values.npy", allow_pickle=True) if os.path.exists("attention_values.npy") else np.empty((0, *alpha_np.shape[1:]))
        # combined_alpha = np.concatenate([existing_alpha, alpha_np])
        # np.save("attention_values.npy", combined_alpha, allow_pickle=True)
...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    ...
    def forward(self, h, x, edge_attr, edge_index, h_bond, bond_index, mask_ligand, e_w=None):
        ...
        new_h_with_edge = self.node_layer_with_edge(h, edge_feat, edge_index, e_w=e_w)  # ht+1' = f(xt, ht) --> AUL edge

        # -> ligand graph message passing
        new_h_with_bond = self.node_layer_with_bond(h, h_bond, bond_index)  # ht+1 '' = f(ht, bt, xt) --> AUL bond
        new_h_bond = h_bond + self.bond_layer(h, h_bond, x, bond_index)  # bt+1 = h(ht, bt, xt) --> BUL 

        # --> update h node
        # new_h = h + self.lin_node(new_h_with_edge + new_h_with_bond)
        l = 0.6  # lambda values (1.6 / 1.4 / 0.6 / 0.4) --> exps 2-5
        new_h = h + self.lin_node(l * new_h_with_edge + (2 - l) * new_h_with_bond)
        # new_h = h + self.lin_node(new_h_with_edge) # used when passing edge and removing bond --> exp 1
        # new_h = h + self.lin_node(new_h_with_bond) # used when passing bond and removing edge --> exp 6

        # update x
        delta_x_with_edge = self.pos_layer_with_edge(new_h, rel_x, edge_feat, edge_index, e_w=e_w)   # g(xt, ht+1, ..) --> PUL edge
        bond_src, bond_dst = bond_index
        rel_bond_x = x[bond_dst] - x[bond_src]
        delta_x_with_bond = self.pos_layer_with_bond(new_h, rel_bond_x, new_h_bond, bond_index) # --> PUL bond
