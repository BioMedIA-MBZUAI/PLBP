...
def obey_lipinski(mol): # Lipinski rule of five used to predict oral bioavailability of drugs 
    mol = deepcopy(mol)
    Chem.SanitizeMol(mol)
    rule_1 = Descriptors.ExactMolWt(mol) < 500 # molecular weight < 500 Daltons
    rule_2 = Lipinski.NumHDonors(mol) <= 5 # hydrogen bond donors 
    rule_3 = Lipinski.NumHAcceptors(mol) <= 10 # hydrogen bond acceptors 
    logp = get_logp(mol)
    rule_4 = (logp >= -2) & (logp <= 5) # logp between -2 and 5
    rule_5 = Chem.rdMolDescriptors.CalcNumRotatableBonds(mol) <= 10 # rotatable bonds
...
def get_logp(mol): # octanol-water partition coefficient 
...
def get_chem(mol):
    ...
    lipinski_score = np.sum(obey_lipinski(mol)) # drug-like molecule
    Chem.GetSymmSSSR(mol) # symmetry-based SSSR (scaffold/substructure/similarity representation) fingerprint 
    ring_info = mol.GetRingInfo() # atom indices of each ring, num of atoms in each ring.  
